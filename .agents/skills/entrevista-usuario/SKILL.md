...
1. **Run `/mycelium:vieses`** before designing questions
...
Draft a scenario entry for `.Codex/canvas/scenarios.yml` from any interview story that is rich enough. Not every interview produces a scenario — only extract when all four elements are present. A partial story is an opportunity, not a scenario.
...
## Output
- Update .Codex/canvas/opportunities.yml with new evidence
- Update .Codex/canvas/user-needs.yml
- Update .Codex/canvas/jobs-to-be-done.yml
- Update .Codex/canvas/scenarios.yml with extracted scenarios (if four elements present)
- Add snapshot to `.Codex/memory/product-journal.md`
