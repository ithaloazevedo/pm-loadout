...
- Update .Codex/canvas/gist.yml goals section
...
- Store in .Codex/canvas/gist.yml idea bank with ICE scores
- Never commit to an idea until evidence supports it

### 3. Score with ICE + Confidence Meter
Use `/mycelium:ice` to prioritize. ICE scoring (Ellis; confidence dimension added by Gilad):
...
> *Mycelium uses 0.0-1.0 (adapted from Gilad's 0-10 non-linear Confidence Meter). See `/mycelium:ice` for details.*
