...
5. **For each leaf opportunity**, check scenario coverage:
   - Does `.Codex/canvas/scenarios.yml` have at least one scenario illustrating this opportunity?
...
7. **For each solution leaf**, assess the Four Risks (Torres Product Trio):
   ...
   Write `four_risks` per solution in `.Codex/canvas/opportunities.yml`.
...
**Always update `.Codex/canvas/opportunities.yml`** with the OST contents after building or updating. This is the single source of truth for the opportunity space.

Also update:
- `.Codex/canvas/scenarios.yml` if scenarios were created or refined (step 5)
- `.Codex/canvas/user-needs.yml` if new needs were identified
- `.Codex/canvas/jobs-to-be-done.yml` if JTBD dimensions surfaced during mapping

## Lean UX Connection

When generating solution ideas for leaf opportunities, frame each as a Lean UX hypothesis:
"We believe [outcome] for [users] if [change]." This makes the solution testable via `/mycelium:suposicoes`.
