...
Discovery/Delivery** — viviam só na lista Iniciativas, hoje depreciada (ver `clickup-config.md`). Não proponha
...
| Delivery | assignee, `_Projeto`, `_Classe`, tipo correto (`Epic` / Tarefa / Bug / Correção — valor exato em clickup-config.md), status inicial correto, linked tasks com Objetivo e Discovery (quando houver) | `_Risco Reg.` se houver exposição regulatória |
