...
## Canvas Output
Record bias check in `.Codex/canvas/opportunities.yml` under `bias_checks` section with date, biases mitigated, and research design adjustments made.

## Decision Log (MANDATORY per G-P4)
**APPEND** a `### Bias Check` entry to `.Codex/harness/decision-log.md` with: activity assessed, active bias risks found, mitigations applied, agent self-check results.
