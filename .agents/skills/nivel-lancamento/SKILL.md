...
## Canvas Output
Update `.Codex/canvas/go-to-market.yml` with tier classification and launch plan.
...
Update `.Codex/canvas/go-to-market.yml` engagement_design section with Hook Canvas results.

*Source: Eyal (Hooked), with ethical framework from the Manipulation Matrix*

## Pre-Launch Bias Check

Before classifying a launch tier, run `/mycelium:vieses` for L5-specific biases:
...
If `/mycelium:vieses` reveals significant biases, address them before finalizing the launch tier.
...
2. **Validate scenarios against reality** (Hoskins):
   - For each scenario in `.Codex/canvas/scenarios.yml` linked to this launch: did the persona's story play out?
...
3. **Evaluate against L2 assumptions**:
   ...
   - Do real user stories suggest NEW scenarios not in `.Codex/canvas/scenarios.yml`?
...
After launch feedback is captured (L5 → L2 loop), update the cycle record in `.Codex/canvas/cycle-history.yml`:

1. Find the cycle record for this leaf (created by `/mycelium:retrospectiva` at delivery completion)
2. Add **actual** market outcomes: user metrics, adoption data, NPS/CSAT, revenue impact
   - **Source this data from `/mycelium:metrics-pull`** where possible (v0.14): 24-48h after launch to capture the bump, then weekly for the first month. Snapshots live at `.Codex/evals/metrics/<source>/*.json`. This replaces manual "I checked the dashboard" reports with timestamped evidence.
   - If `.Codex/jit-tooling/active-metrics.yml` has no configured source for the relevant channel, run `/mycelium:metricas` first.
...
**APPEND** a `### Launch Tier Classification` entry to `.Codex/harness/decision-log.md` with: tier assigned, positioning rationale, key risks, go-to-market approach.
