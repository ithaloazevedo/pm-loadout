...
> **Fonte canônica do tom de voz: `knowledge/tom-de-voz.md`.** Este arquivo é a aplicação específica
> daquelas regras ao corpo de um card. Em caso de divergência, o guia de tom de voz prevalece.

Regras de como escrever qualquer seção narrativa de um card (Contexto, comentários de update) — em
Discovery, Delivery e tickets operacionais. Aplica a todo agente que redige corpo de card:
`agente-delivery`, `agente-spec`.
...
## Recap antes de responder — quem não acompanhou o histórico não pode ficar perdido

Quando um comentário de update responde a perguntas ou hipóteses já levantadas antes (numa
investigação em andamento, num caso técnico, num card com anexos/relatórios anteriores), não abra
direto pela resposta técnica. Abra com um recap curto do caso — o suficiente para alguém que só
viu a cobrança, sem ter lido o relatório anterior, entender do que se trata — e só depois responda
cada pergunta.

Cada achado carrega o número que o sustenta **junto da frase**, não só no anexo/planilha:

> ❌ `A hipótese da compra de bônus não se sustenta.`
> ✅ `278 dos 374 casos (74%) nunca apostaram nesse jogo — a hipótese de "compra de bônus" não
> explica a maioria.`

Quando há mais de uma pergunta em aberto sendo respondida na mesma mensagem, trate cada uma como um
bloco: pergunta (ou o que motivou a dúvida) → resposta com o dado que a sustenta → o que ainda fica
em aberto, se for o caso. Feche com uma conclusão consolidada e o próximo passo — não deixe a
mensagem terminar no último achado.
...
| Seção | Como aplicar |
|---|---|
| 🎯 Objetivo | 1-2 frases, mas ainda assim problema → valor, não só a ação |
...
| Comentários de update | Contexto → decisão/fato → próximo passo (padrão em [clickup-method.md](clickup-method.md)); se responde a perguntas de um caso em andamento, recap do caso + número junto de cada achado antes da conclusão |
