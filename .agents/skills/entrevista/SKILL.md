...
Read `.Codex/canvas/purpose.yml` and `.Codex/diamonds/active.yml` at session start. Determine state:
...
Output the brief markdown to the chat. This is the visible payoff and it MUST appear before canvas writes — Codex clutters the TUI with tool-call blocks if writes come first.
...
- `.Codex/canvas/purpose.yml`: purpose statement from Q1, JTBD functional from Q1+Q2, workarounds from Q2. Tag all entries `source_class: internal_stakeholder, validated: false`.
- `.Codex/diamonds/active.yml`: L0 Purpose diamond, phase Discover, `confidence: 0.15` (hardcoded floor for brief-only state — see DEFERRED note at end of file), evidence_type: internal_stakeholder, theory_gates_status all pending, note: `created_via: brief`.
...
After writes, output one line: `Saved your brief to canvas (purpose.yml + .Codex/diamonds/active.yml).`
...
1. Test the biggest assumption  (~10 min)  — /mycelium:suposicoes on what you flagged as risky. Smallest-viable-test design.
...
| "complex," "uncertain," "novel," "no precedent," "first time anyone has" | Classify the domain (`/mycelium:cynefin`) | ~5 min |
...
- **Test the biggest assumption** → Invoke `/mycelium:suposicoes` with Q3's biggest assumption pre-loaded as the target. Confidence will refine when the test designs.
...
| Brief mentions "first," "novel," "no one has done this" | Phase 4 Landscape (Wardley genesis lens) + suggest `/mycelium:cynefin` | Greenfield needs strategic frame |
...
When `/mycelium:entrevista` is invoked on a canvas with content, do not run the brief flow. Instead:
...
**All `/mycelium:entrevista` outputs are stakeholder beliefs, not validated evidence.** This is a stakeholder interview — the founder/PM is sharing their mental model of the product, users, and market.
...
Store as `product_type` on the L0 diamond entry in `.Codex/diamonds/active.yml` (per-diamond field, not root-level). Child diamonds inherit product_type from their parent unless overridden. Load delivery profile from `${CLAUDE_PLUGIN_ROOT}/engine/canvas-guidance.yml#product_types`.
...
- Set `dogfood: true` in `.Codex/diamonds/active.yml` (in addition to `project_type`)
...
- Reference `${CLAUDE_PLUGIN_ROOT}/engine/dogfood-mode.md` for the pattern (when/how to run, report structure, anonymization rules). Reports themselves go to `.Codex/evals/dogfood-reports/` in the user's project.
...
Store classification in `.Codex/diamonds/active.yml` as `project_type`. If dogfood, also store `dogfood: true`.
...
- software -> `.Codex/canvas/dora-metrics.yml` (already exists as template)
- content_course/content_publication/content_media -> `.Codex/canvas/content-metrics.yml`
- ai_tool -> `.Codex/canvas/ai-tool-metrics.yml`
- service_offering -> `.Codex/canvas/service-metrics.yml`
...
4. Write `.Codex/jit-tooling/active-metrics.yml`.

Tell the user: "I've configured N metric source(s) in `.Codex/jit-tooling/active-metrics.yml`. Run `/mycelium:metrics-pull` whenever you want a fresh snapshot — I'll also remind you before `/mycelium:diamond-assess` at L0/L1/L2/L5 if the latest is >7 days old."

If the user prefers to defer this: skip, note in the interview summary, and suggest `/mycelium:metricas` later.
...
**CRITICAL**: After the interview, Mycelium must stay present — but lightweight. The process cliff (corrections.md, 2026-04-30) happens when the agent drops all framework structure after /mycelium:entrevista and becomes a raw implementation co-pilot.
...
3. **Run gates inline**, not as separate skill invocations. Instead of "/mycelium:checar-servico" as a formal step, weave the Downe principles check into the delivery review naturally.
...
*Source: Hoskins transcript (2026-04-25) — process abandoned for 75% of session after /mycelium:entrevista. Böckeler (inferential guidance that's too heavy gets ignored). Smart (BVSSH Sooner — process overhead that slows delivery without adding value is waste). Corrections.md: "Process cliff after onboarding."*
