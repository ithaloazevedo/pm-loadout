...
### Step 1. Record Cycle in `.Codex/canvas/cycle-history.yml` AND Decision Log (MANDATORY — DO THIS FIRST)

**This step is critical.** Without it, the learning metabolism has no data. You MUST do BOTH parts (5a and 5b).

#### Step 5a. Write cycle record to `.Codex/canvas/cycle-history.yml`

Find the leaf_id and opportunity_id for the delivered solution (from `.Codex/canvas/opportunities.yml` or `.Codex/canvas/gist.yml`). Then write a cycle record:

```yaml
- cycle_id: cycle-NNN
  leaf_id: "opp-XXX-sol-X"         # From opportunities.yml
  opportunity_id: "opp-XXX"         # Parent opportunity
  diamond_id: "d-XXX"               # From .Codex/diamonds/active.yml
...
#### Step 5b. Log cycle calibration summary in .Codex/harness/decision-log.md
...
Update the cycle record in `.Codex/canvas/cycle-history.yml` with the rework fields. This is the denominator — the hidden cost of delivery that velocity metrics miss.

If this retrospective is for a just-completed cycle, prompt: "Set a reminder to check rework in 14 days. Run `/mycelium:retrospectiva rework-check [cycle-id]` after that."
...
## Output
1. Update `.Codex/memory/corrections.md` with new corrections
2. Update `.Codex/memory/patterns.md` with new patterns
3. Update `.Codex/memory/delivery-journal.md` with retrospective entry
4. Update .Codex/canvas/bvssh-health.yml if dimensions changed
5. Log in .Codex/harness/decision-log.md
6. Record cycle in `.Codex/canvas/cycle-history.yml` (see Cycle History Recording above)

## Counter-Argument Check (Bias Mitigation)

Before finalizing the retrospective, draft a one-line counter-argument for each major claim: *"What's the strongest case that this 'went well' was actually luck? That this 'went wrong' was actually unavoidable? That this 'pattern' is actually noise?"* If you can't articulate counter-cases, run `/mycelium:advogado-do-diabo` before locking in the corrections/patterns.
