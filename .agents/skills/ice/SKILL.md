...
> **Compatibilidade Codex.** Referências herdadas a `Read`/`Write`/`Edit`, `/mycelium:*`, `.claude/`, `.Codex/` ou `CLAUDE_PLUGIN_ROOT` são integrações legadas e opcionais. No Codex, inspecione arquivos antes de alterá-los e use `apply_patch`; não crie esses diretórios legados. Entregue o artefato na conversa e só persista conteúdo em `knowledge/` ou no sistema externo apropriado quando isso for solicitado ou autorizado. Quando uma skill legada indicar outra `/mycelium:*`, use a skill local equivalente pelo nome, se estiver disponível.
...
For each solution leaf in `.Codex/canvas/opportunities.yml`, check that `four_risks` has been assessed:
...
For the top-ranked solutions, extract the **highest-risk assumptions** from the Four Risks assessment — these are what `/mycelium:suposicoes` should target first. Prioritize assumptions where importance is high but evidence is low.
...
## Canvas Output
Update `.Codex/canvas/opportunities.yml` — write `four_risks` and `ice_score` per solution leaf.
Update `.Codex/canvas/gist.yml` with idea ICE scores and confidence levels.
