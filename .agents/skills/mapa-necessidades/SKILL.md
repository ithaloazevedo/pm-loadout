...
## Canvas Output
**Always update** `.Codex/canvas/user-needs.yml` with discovered needs, scores, and states.
Also update:
- `.Codex/canvas/opportunities.yml` with opportunity areas derived from underserved needs
- `.Codex/canvas/landscape.yml` if needs mapping reveals new value chain components
- `.Codex/canvas/team-shape.yml` if need clusters suggest different team boundaries

## Bias Warning
Before mapping needs, run `/mycelium:vieses`. Key biases:
