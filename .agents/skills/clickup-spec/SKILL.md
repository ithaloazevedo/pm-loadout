...
invocation: model
inputs: item do ClickUp, contexto de produto e conectores autorizados
outputs: proposta, validação ou alteração confirmada do item
side_effects: write-confirmed
context: knowledge/domains/processo.md, references/, config/pm-loadout.local.yaml quando existir
completion: item avaliado ou proposta entregue; toda escrita externa confirmada pelo usuário
---
...
**Gate de configuração:** se `config/pm-loadout.local.yaml` existir e `workspace.clickup.enabled` estiver como `false`, não chame o conector. Explique que o workspace está em modo offline e entregue a proposta de item na conversa. Só retome o conector após o usuário habilitar a configuração local.

**Se responder:** prossiga. **Se não estiver disponível:** interrompa e exiba:

> Para usar esta skill, conecte o ClickUp ao Codex e verifique a disponibilidade do conector antes de ler ou gravar dados.
>
...
| Projeto de Delivery | tipo confirmado, subtipo (`Epic` / Tarefa / Bug / Correção — valor exato em clickup-config.md), Squad confirmado (para escolher o folder), escopo fechado (capacidades) |
...
Se sim → identifique o folder correto pelo **Squad** responsável → crie no **Backlog** desse folder (status `backlog`), `clickup_create_task` com tipo correto (`Epic` por default) → **linked task** Delivery ↔ Discovery e (quando houver) Delivery ↔ Objetivo. Retorne o link.
...
### Passo 2: Analisar em dois eixos independentes

Não deixe um card bem formatado mascarar uma decisão fraca, nem uma decisão sólida mascarar um item impossível de executar. Reporte os dois eixos separadamente; não some notas.

**Eixo A — Fidelidade à decisão e evidência**

| Critério | Status |
|---|---|
| Problema, objetivo e público afetado estão claros? | ✅/❌/NA |
| Afirmações factuais relevantes têm dado, decisão registrada ou fonte vinculada? | ✅/❌/NA |
| Solução e escopo respondem ao problema declarado? | ✅/❌ |
| Suposições, riscos regulatórios e decisões ainda abertas estão explicitados quando relevantes? | ✅/❌/NA |
| Não há compromisso novo inventado pelo agente ou não confirmado pelo PM? | ✅/❌ |

**Eixo B — Prontidão do artefato**
...
| Dono nomeado (assignee) ou pendência de dono registrada? | ✅/❌ |
...
| Dono nomeado ou pendência de dono registrada? | ✅/❌ |
...
```markdown
## Avaliação Final
### Fidelidade à decisão e evidência
[🟢 Sustentada | 🟡 Lacunas a resolver | 🔴 Não sustentada]
[achado mais importante]

### Prontidão do artefato
[🟢 Pronto | 🟡 Ajustes menores | 🔴 Reestruturar]
[achado mais importante]

**Próximo passo**: [uma ação concreta que resolve o eixo pior]
```
