...
## Canvas Output
Update `.Codex/diamonds/active.yml` with the `cynefin_domain` field for the relevant diamond.
If Wardley mapping was referenced, update `.Codex/canvas/landscape.yml` component evolution stages.
...
**APPEND** a `### Cynefin Classification` entry to `.Codex/harness/decision-log.md` with: domain classified, key indicators, method routed to, confidence in classification.
